...
public:
    ...
    string RotateString2(string &str, int left, int right) {
        ...
        string res;
        int offset = (left > right)? left - right: right - left;
        int n = str.size(); offset %= n;
        if (left > right)
           res = str.substr(offset) + str.substr(0, offset);
        else
           res = str.substr(n - offset) + str.substr(0, n - offset);
        return res;
    }
};